...
const give_all_gold_arena_ticket = function() {
  ...
  .then(function(){
    ...
    return knex('users')
      .count('id')
    .then(results => results[0].count);}).then(function(userCount) {
    ...
    bar.total = numUsers;
    return results.numUsers = numUsers;}).then(() => _processNextSetOfUsers(batchIndex));
};
...
var _processNextSetOfUsers = function(batchIndex) {
  ...
  .then(function(users) {
        ...
        .then(function(){
          if (dryRun) {
            ...
            var txPromise = knex.transaction(function(tx){
              ...
              .then(userRow => GiftCrateModule.addGiftCrateToUser(txPromise,tx,userId,GiftCrateLookup.FebruaryLag2016)).then(tx.commit)
              .catch(tx.rollback);
            });
            return txPromise;
          }}).then(function(){
    ...
    ,{concurrency:8});}).catch(function(e){

    return this.errors.push(e);}).then(function(needsMoreProcessing) {
