          ...
          if (questData) {
            if (questData.daily && questData.daily.current && questData.daily.current.quests) {
              ...
              for (var questIndex in currentQuests) {
                var questEntryData = currentQuests[questIndex];
                var quest = QuestFactory.questForIdentifier(questEntryData.q_id);
              ...
              while (validQuests.length < targetNumberOfQuests) {
                // add a quest
                var newQuest = QuestFactory.randomQuestForSlotExcludingIds(validQuests.length,questIds);
                var newQuestData = UsersModule.firebaseQuestDataForQuest(newQuest);
