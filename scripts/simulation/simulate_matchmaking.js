...
const addSomePlayers = function(c){
  ...
  return (() => {
    const result = [];
    for (let i = 0, end = c, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
      var token = Redis.TokenManager.create({
      ...
      var division = __guard__(RankFactory.rankedDivisionAssetNameForRank(token.rank/10), x => x.toLowerCase());
...
const startWorker = function(){
  ...
  */
  const cleanShutdown = () => worker.shutdown(function(err) {
    if (err) {
