...
const migrateFromStagingToCurrentEnvironment = function(stagingToken,dateKey){
  ...
  .spread(function(stagingRootRef,localRootRef) {
    ...
    return Promise.resolve(fetch(stagingChallengeURL));}).then(function(res){
    ...
    return zlib.gzipAsync(JSON.stringify(data));}).then(function(gzipGameSessionData) {
    ...
    return FirebasePromises.set(this.localRootRef.child("daily-challenges").child(dateKey),this.dailyChallengeData);}).then(() => Logger.module("Script").log("Migration complete"));
};
