...
let chestType, cosmeticData, cosmeticId, cosmeticTypeKey, factionId, factionIdKey, rarityId, rarityIdKey, type;
...
const EmoteCategory = require('./emoteCategory');
...
class CosmeticsFactory {
  ...
  static cosmeticForSku(sku) {
    const cosmeticId = parseInt(sku.slice(sku.lastIndexOf("-")+1));
    return this.cosmeticForIdentifier(cosmeticId);
  }
  static getAllCosmetics() {
    return this._cachedCosmetics;
  }
  static cosmeticsForType(cosmeticType) {
    return this._cachedCosmeticsForType[cosmeticType];
  }
  static cosmeticsForTypeAndFaction(cosmeticType, factionId) {
    return this._cachedCosmeticsForTypeAndFaction[cosmeticType][factionId];
  }
  static cosmeticsForTypeAndRarity(cosmeticType, rarityId) {
    return this._cachedCosmeticsForTypeAndRarity[cosmeticType][rarityId];
  }
  static cosmeticsForTypeAndFactionAndRarity(cosmeticType, factionId, rarityId) {
    return this._cachedCosmeticsForTypeAndFactionAndRarity[cosmeticType][factionId][rarityId];
  }
  static cosmeticsForFaction(factionId) {
    return this._cachedCosmeticsForFaction[factionId];
  }
  static cosmeticsForRarity(rarityId) {
    return this._cachedCosmeticsForRarity[rarityId];
  }
  static cosmeticsForChestType(chestType) {
    return this._cachedCosmeticsForChestType[chestType];
  }
  static visibleCosmeticSubTypesForType(cosmeticType) {
    let orderedSubTypes = [];
    ...
    const allSubtypesForType = this._cachedCosmeticSubTypesByType[cosmeticType];
    const subTypesNotInOrderedList = _.difference(allSubtypesForType,orderedSubTypes);

    // Combine ordered and any missing subtypes
    const visibleSubTypes = orderedSubTypes.concat(subTypesNotInOrderedList);
  ...
  static localizedSubTypeTitle(subTypeId) {
    ...
    if (this._cachedLocalizedSubTypeTitles[subTypeId] != null) {
      return this._cachedLocalizedSubTypeTitles[subTypeId];
    } else {
//      console.warn "No localized string for subtype: " + subTypeId
      return subTypeId;
    }
  }
  static cosmeticsForTypeAndSubtype(type,subType) {
    return this._cachedCosmeticsForTypeAndSubType[type][subType];
  }
  ...
  static profileIconForIdentifier(identifier) {
    ...
    if ((cosmeticData != null) && (cosmeticData.typeId === CosmeticsTypeLookup.ProfileIcon)) {
      ...
      return this.cosmeticForIdentifier(this.getDefaultProfileIconIdentifier());
    }
  }
  ...
  static cardBackForIdentifier(identifier) {
    ...
    if ((cosmeticData != null) && (cosmeticData.typeId === CosmeticsTypeLookup.CardBack)) {
      ...
      return this.cosmeticForIdentifier(this.getDefaultCardBackIdentifier());
    }
  }
  ...
  static sceneForIdentifier(identifier) {
    ...
    if ((cosmeticData != null) && (cosmeticData.typeId === CosmeticsTypeLookup.Scene)) {
      ...
      // return default card back
      return this.cosmeticForIdentifier(this.getDefaultSceneIdentifier());
    }
  }
  ...
  static cardSkinIdsForCard(cardId) {
    return this._cachedCardSkinIdsForCardId[cardId] || [];
  }
  ...
  static cosmeticProductDataForIdentifier(identifier) {
    const cosmeticData = this.cosmeticForIdentifier(identifier);
    if (cosmeticData != null) {
      return this._mapCosmeticDataToProductData(cosmeticData);
    }
  }
  ...
  static cosmeticProductDataForType(cosmeticType,cosmeticSubType) {
    ...
    cosmeticDatas = _.filter(cosmeticDatas, c => c.purchasable && c.rarityId);
  ...
  static _mapCosmeticDataToProductData(cosmeticData){
    const rarityData = RarityFactory.rarityForIdentifier(cosmeticData.rarityId);
    const cosmeticRSX = cosmeticData.rsx || {};
    const cosmeticCoverRSX = cosmeticData.coverRSX || RSX.challenge_gate_010;
    const cosmeticAnimResource = cosmeticData.animResource || {};
    return {
      id: cosmeticData.id,
  ...
  static cosmeticResourcesForIdentifier(identifier) {
    const resources = [];
    ...
    if (cosmeticData != null) {
      ...
      if (cosmeticData.animResource != null) {
        for (var animResourceKey of Array.from(Object.keys(cosmeticData.animResource))) {
  ...
  static cosmeticProductAttrsForIdentifier(identifier){
    const cosmeticData = this.cosmeticForIdentifier(identifier);
    if (cosmeticData) {
      return this._mapCosmeticDataToProductData(cosmeticData);
    } else {
  ...
  static cosmeticProductAttrsForSKU(sku){
    const cosmeticData = this.cosmeticForSku(sku);
    if (cosmeticData) {
      return this._mapCosmeticDataToProductData(cosmeticData);
    } else {
      return null;
  ...
  static descriptionForCosmeticChestType(cosmeticCrateType){
    if (cosmeticCrateType === CosmeticsChestTypeLookup.Common) {
      return [
...
const ORDER_EMOTES = 0;
const ORDER_CARD_BACKS = 1;
const ORDER_PROFILE_ICONS = 0;
const ORDER_MAIN_MENU_PLATES = 1;
const ORDER_CARD_SKINS = 2;
const ORDER_SCENES = 0;

// endregion COSMETICS ORDERING

// region EMOTE ORDERING

const EMOTE_ORDER_HAPPY = 0;
const EMOTE_ORDER_ANGRY = 1;
const EMOTE_ORDER_CONFUSED = 2;
const EMOTE_ORDER_SAD = 3;
const EMOTE_ORDER_TAUNT = 4; // wink
const EMOTE_ORDER_KISS = 5; // love
const EMOTE_ORDER_FRUSTRATED = 6;
const EMOTE_ORDER_SURPRISED = 7;
const EMOTE_ORDER_BOW = 8;
const EMOTE_ORDER_SLEEP = 9;
const EMOTE_ORDER_SUNGLASSES = 10;
const EMOTE_ORDER_CUSTOM = 11;
...
const cosmeticTypes = Object.keys(CosmeticsTypeLookup);
const cosmeticChestTypes = Object.keys(CosmeticsChestTypeLookup);
const factionIds = Object.keys(Factions);
const rarityIds = Object.keys(Rarity);

for (cosmeticTypeKey of Array.from(cosmeticTypes)) {
  ...
  for (factionIdKey of Array.from(factionIds)) {
    ...
    for (rarityIdKey of Array.from(rarityIds)) {
...
for (var cosmeticChestTypeKey of Array.from(cosmeticChestTypes)) {
...
const cosmeticIdKeys = Object.keys(CosmeticsFactory._cosmeticsById);
for (var cosmeticIdKey in cosmeticIdKeys) {
...
const allCosForDesc = CosmeticsFactory.getAllCosmetics();
for (var cosForDesc of Array.from(allCosForDesc)) {
  if (cosForDesc.shopDescription != null) {
    continue;
  }
...
for (var cosForOrder of Array.from(allCosForRewardOrder)) {
