...
class SisterAchievement extends Achievement {
  static initClass() {
    this.id = "sandSister";
    this.title = i18next.t("achievements.sand_sister_title");
    this.description = i18next.t("achievements.sand_sister_desc");
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount = (cardCollection[Cards.Faction3.SandSister] != null ? cardCollection[Cards.Faction3.SandSister].count : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }

    // check if player owns 3 or more of at least 6 rare cards for this faction
    const allFactionRares = _.filter(allCards,card => (card.getFactionId() === Factions.Faction3) &&
