...
class SisterAchievement extends Achievement {
  static initClass() {
    this.id = "lightningSister";
    this.title = i18next.t("achievements.storm_sister_title");
    this.description = i18next.t("achievements.storm_sister_desc");
    this.progressRequired = 1;
    this.rewards = {
      cards: [
        Cards.Faction2.LightningSister,
        Cards.Faction2.LightningSister,
        Cards.Faction2.LightningSister
      ]
    };
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount = (cardCollection[Cards.Faction2.LightningSister] != null ? cardCollection[Cards.Faction2.LightningSister].count : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }

    // check if player owns 3 or more of at least 6 rare cards for this faction
    const allFactionRares = _.filter(allCards,card => (card.getFactionId() === Factions.Faction2) &&
