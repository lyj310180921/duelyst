...
class SisterAchievement extends Achievement {
  static initClass() {
    this.id = "sunSister";
    this.title = i18next.t("achievements.sun_sister_title");
    this.description = i18next.t("achievements.sun_sister_desc");
    this.progressRequired = 1;
    this.rewards = {
      cards: [
        Cards.Faction1.SunSister,
        Cards.Faction1.SunSister,
        Cards.Faction1.SunSister
      ]
    };
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount = (cardCollection[Cards.Faction1.SunSister] != null ? cardCollection[Cards.Faction1.SunSister].count : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }

    // check if player owns 3 or more of at least 6 rare cards for this faction
    const allFactionRares = _.filter(allCards,card => (card.getFactionId() === Factions.Faction1) &&
