...
GameSetup.setupNewSession = function(gameSession, player1Data, player2Data, withoutManaTiles) {
  ...
  if (gameSession != null) {
    ...
    // get players
    let battemapTemplateIndex, general1X, general1Y, general2X, general2Y;
...
GameSetup.addCardsToBoard = function(gameSession, boardCardsData, owner) {
  if (boardCardsData != null) {
    ...
    return (() => {
      const result = [];
      for (var cardData of Array.from(boardCardsData)) {
...
GameSetup.createGameSetupData = function(gameSession, player1Data, player2Data) {
  ...
  for (var card of Array.from(gameSession.getBoard().getCards(null, (allowUntargetable=true)))) {
...
GameSetup.createGameSetupDataForPlayer = function(gameSession, player, playerData) {
  ...
  playerGameSetupData.startingHand = _.map(playerDeck.getCardsInHand(), function(card) {
    if (card != null) {
