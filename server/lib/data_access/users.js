...
const grantFullCollection = require('../collection.coffee');
...
class UsersModule {
  ...
  static createNewUser(username,password,inviteCode,referralCode,campaignData,registrationSource = null){
    ...
    .then(function(inviteCodeRow){
      ...
      return hashHelpers.generateHash(password);}).then(function(passwordHash){
  ...
  static deleteNewUser(userId) {
    ...
    .then(function(userRow) {
      ...
      return knex("users").where('id',userId).delete();}).then(() => DuelystFirebase.connect().getRootRef())
    .then(function(rootRef) {
      const promises = [
      ...
      if (existingUserId) {
        ...
        return knex.transaction(function(tx){
          ...
          .then(function(userRow){
            ...
            return Promise.all(allUpdates);}).then(() => DuelystFirebase.connect().getRootRef())
          .then(function(rootRef){
  ...
  static bumpSessionCountAndSyncDataIfNeeded(userId,userData=null,systemTime=null){
    ...
    .then(function(userData){
      ...
      return MigrationsModule.checkIfUserNeedsMigrateEmotes20160708(this.userData);}).then(function(userNeedsMigrateEmotes) {
      if (userNeedsMigrateEmotes) {
        return MigrationsModule.userMigrateEmotes20160708(userId,MOMENT_NOW_UTC);
      } else {
        return Promise.resolve();
      }}).then(function() {
      return MigrationsModule.checkIfUserNeedsPrismaticBackfillReward(this.userData);}).then(function(userNeedsPrismaticBackfill) {
      ...
      return SyncModule.syncUserDataIfTrasactionCountMismatched(userId,this.userData);}).then(synced => // # Job: Sync user buddy data
...
//        return Promise.resolve(true)
      ...
      const oldDeckPromise = knex("user_gauntlet_run_complete").first("deck","ended_at").where("user_id",userId).andWhere("id",ticketId).andWhere("ended_at",">",decksExpireMoment.toDate());
      ...
      .spread(function(currentRunRow,completedRunRow) {
        ...
        for (let i = 0, end = sortedDeckIds.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  static createFactionProgressionRecord(userId,factionId,gameId,gameType,systemTime) {
    ...
    .then(function(rootRef){
      ...
      return FirebasePromises.set(rootRef.child('user-faction-progression').child(userId).child(factionId).child('stats'),this.factionProgressionRow);}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
    .timeout(10000)
...
//        xp_earned = if isWinner then SDK.FactionProgression.winXP else SDK.FactionProgression.lossXP
      ...
      return Promise.resolve();}).then(() => DuelystFirebase.connect().getRootRef())
    .then(function(rootRef){
    ...
    .then(function(){
      ...
      return this.factionProgressionRow;}).catch(Errors.MaxFactionXPForSinglePlayerReachedError, function(e){
...
//            @.hasReachedDailyWinCountBonusLimit = true
  ...
  static updateUserBossProgressionWithGameOutcome(userId,opponentId,isWinner,gameId,gameType,isUnscored,isDraw,gameSessionData,systemTime) {
    ...
    .then(function(userRow) {
      ...
      return FirebasePromises.once(bossEventsRef,'value');}).then(function(bossEventsSnapshot){
      ...
      return Promise.all([
        this.userRow,
        tx('user_bosses_defeated').where('user_id',userId).andWhere("boss_id",bossId).andWhere("boss_event_id",this.matchingEventId).first()
...
//        allPromises.push(InventoryModule.giveUserGold(txPromise,tx,userId,rewardData.gold,rewardData.reward_type))
  ...
  static updateGameCounters(userId,factionId,generalId,isWinner,gameType,isUnscored,isDraw,systemTime) {
    ...
    .then(userRow => Promise.all([
      ...
      return Promise.all(allPromises);}).timeout(10000)
    .catch(Promise.TimeoutError, function(e){
  ...
  static updateUserStatsWithGame(userId,gameId,gameType,gameData,systemTime) {
    ...
    .then(function(fbRootRef) {
      ...
      return new Promise((resolve, reject) => statsRef.once("value", statsSnapshot => resolve(statsSnapshot.val())));}).then(function(statsData) {
      let statsRef;
      try {
      ...
      .spread(function(quests,questsComplete){
        ...
        if ((beginnerQuestsComplete != null ? beginnerQuestsComplete.length : undefined) < beginnerQuests.length) {
          ...
          .then(function(questData){
            ...
            }}).catch(Errors.NoNeedForNewBeginnerQuestsError, e => Logger.module('SDK').debug(`iterateNewPlayerCoreProgression() -> no need for new quests at ${nextStage.key} for ${userId.blue}`)).then(function(){
        ...
        .then(function(progressionData) {
          ...
          return QuestsModule.generateBeginnerQuests(userId);}).then(function(questData){
          if (questData) {
            return this.questData = questData;
          }}).catch(Errors.NoNeedForNewBeginnerQuestsError, e => Logger.module('SDK').debug(`iterateNewPlayerCoreProgression() -> no need for new quests at ${nextStage.key} for ${userId.blue}`)).then(function(){
          return this;
  ...
  static setNewPlayerFeatureProgression(userId,moduleName,stage) {
    ...
    const txPromise = knex.transaction(function(tx){
      ...
      .then(function(progressionRow){
        ...
        return queryPromise;}).then(function(updateCount){
        if (updateCount) {
  ...
  static setPortraitId(userId,portraitId) {
    ...
    return txPromise = knex.transaction(function(tx){
      ...
      .then(rootRef => FirebasePromises.set(rootRef.child('users').child(userId).child('presence').child('portrait_id'),portraitId)).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
  ...
  static setBattleMapId(userId,battleMapId) {
    ...
    return txPromise = knex.transaction(function(tx){
      ...
      .then(rootRef => FirebasePromises.set(rootRef.child('users').child(userId).child('battle_map_id'),battleMapId)).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
