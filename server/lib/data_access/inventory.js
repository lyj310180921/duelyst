...
class InventoryModule {
  ...
  static giveUserGold(trxPromise,trx,userId,goldAmount,memo,sourceId){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`giveUserGold() -> User ${userId.blue}`.green + ` received ${goldAmount} gold.`.green));
  }
  ...
  static debitGoldFromUser(trxPromise,trx,userId,goldAmount,memo,sourceId){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`debitGoldFromUser() -> User ${userId.blue}`.green + ` received ${goldAmount} gold.`.green));
  }
  ...
  static giveUserSpirit(trxPromise,trx,userId,spiritAmount,memo){
    ...
    .then(() => DuelystFirebase.connect().getRootRef()).then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`giveUserSpirit() -> User ${userId.blue}`.green + ` received ${spiritAmount} spirit.`.green));
  }
  ...
  static giveUserPremium(trxPromise,trx,userId,amount,memo){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`giveUserPremium() -> User ${userId.blue}`.green + ` received ${amount}.`.green));
  }
  ...
  static debitPremiumFromUser(trxPromise,trx,userId,amount,memo){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`debitPremiumFromUser() -> User ${userId.blue}`.green + ` received ${amount}.`.green));
  }
  ...
  static giveUserNewPurchasableCosmetic(trxPromise,trx,userId,transactionType,transactionId,rarityId,cosmeticType,cosmeticIdsOwned,systemTime){
    ...
    if (cosmeticIdsOwned != null) {
      ...
      currentCosmeticInventoryPromise = currentCosmeticInventoryPromise.then(function(cosmeticRows) {
        cosmeticIdsOwned = [];
    ...
    .then(function(cosmeticIdsOwned) {
      ...
      return InventoryModule.giveUserCosmeticId(trxPromise,trx,userId,cosmeticIdToGive,transactionType,transactionId,null,systemTime);}).then(function(cosmeticRewardData) {
  ...
  static isAllowedToUseCards(txPromise, tx, userId, cardIds) {
    ...
    .spread(function(factionProgression,cardCollectionRow){
      ...
      for (var cardIdKey in cardIds) {
        var numCards = cardIds[cardIdKey].length;
        var cardId = parseInt(cardIdKey);
        var nonSkinnedCardId = SDK.Cards.getNonSkinnedCardId(cardId);

        // check for cards unlocked via progression
        var levelRequiredForCard = SDK.FactionProgression.levelRequiredForCard(nonSkinnedCardId);
        if (levelRequiredForCard > 0) {
          var factionRequiredForCard = SDK.FactionProgression.factionRequiredForCard(nonSkinnedCardId);
          factionLevel = factionLevel_FactionId[factionRequiredForCard];
          var unlockedCardIds = SDK.FactionProgression.unlockedCardsUpToLevel(factionLevel, factionRequiredForCard);
    ...
    .spread(function(factionProgression,cardCollectionRow){
      ...
      for (var cardIdKey in cardIds) {
        ...
        if (levelRequiredForCard > 0) {
          ...
          if (_.contains(unlockedCardIds, nonSkinnedCardId)) {
            var asc, end;
            for (i = 0, end = numCards, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        ...
        if (cardCollectionRow != null) {
          var cardData = cardCollectionRow.cards[cardIdKey];
          if (cardData != null) {
            var asc1, end1;
            for (i = 0, end1 = Math.min(cardData.count, numCards), asc1 = 0 <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
              usableCards.push(cardId);
            }
          }
        }
  ...
  static debitSpiritFromUser(trxPromise,trx,userId,spiritAmount,memo,sourceId){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateWalletData);}).then(() => Logger.module("InventoryModule").timeEnd(`debitSpiritFromUser() -> User ${userId.blue}`.green + ` received ${spiritAmount} spirit.`.green));
  }
  ...
  static buyBoosterPacksWithGold(userId, qty, cardSetId, sku) {
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .then(function(userRow){
        ...
        const all = [];
        for (let i = 0, end = qty, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
          all.push(InventoryModule.addBoosterPackToUser(txPromise,tx,userId,cardSetId,"soft"));
        }
        return Promise.all(all);}).then(function(boosterIds){
  ...
  static addBoosterPackToUser(trxPromise, trx, userId, cardSetId, transactionType, transactionId=null, additionalBoosterAttrs=null,systemTime){
    ...
    .then(function(){
      ...
      return orbCountTrackingPromise;}).then(() => DuelystFirebase.connect().getRootRef())
    .then(function(fbRootRef) {
  ...
  */
  static addRemainingOrbsForCardSetToUser(txPromise, tx, userId, cardSetId, refundWithSpirit, transactionType, transactionId=null, systemTime){
    ...
    .then(function(userRow) {
      ...
      for (let i = 1, end = this.orbsToGive, asc = 1 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
      ...
      return Promise.all(allPromises);}).then(() => Logger.module("InventoryModule").timeEnd(`addRemainingOrbsForCardSetToUser() -> added complete set for ${cardSetId} to user ${userId.blue}.`.green));
  }
  ...
  static unlockBoosterPack(userId, boosterPackId,systemTime) {
    ...
    return txPromise = knex.transaction(function(tx){
          ...
          try {
            ...
            if (this.cardSetId === SDK.CardSet.Core) {
              ...
              for (i = 1; i <= 4; i++) {
                random = Math.random();
                if (random < 0.73) {
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getIsLegacy(false).getCardIds(),new_cards,0.04,50));
                } else if (random < 0.88) {
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getIsLegacy(false).getCardIds(),new_cards,0.06,50));
                } else if (random < 0.98) {
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getIsLegacy(false).getCardIds(),new_cards,0.07,50));
                } else {
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getIsLegacy(false).getCardIds(),new_cards,0.08,50));
                }
              }
              ...
              for (i = 1; i <= 4; i++) {
                ...
                if (random < 0.74) {
                  ...
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,shimzarCommonPrismaticChance,50));
                } else if (random < (0.74 + 0.16)) {
                  // 16% chance for rare
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,shimzarRarePrismaticChance,50));
                } else if (random < (0.74 + 0.16 + 0.09)) {
                  // 9% chance for epic
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,shimzarEpicPrismaticChance,50));
                } else {
                  // 1% chance for legendary
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,shimzarLegendaryPrismaticChance,50));
                }
              }
              ...
              const commonCardId = randomUnownedCardFromCollection(this.userCardsData,SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsUnlockable(true).getIsPrismatic(false).getCardIds());
              new_cards.push(commonCardId);
              new_cards.push(commonCardId);
              new_cards.push(commonCardId);

              // 3 Of an unowned rare
              const rareCardId = randomUnownedCardFromCollection(this.userCardsData,SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsUnlockable(true).getIsPrismatic(false).getCardIds());
              ...
              const epicBloodbornCardIds = SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsUnlockable(true).getIsPrismatic(false).getCardIds();
              const legendaryBloodbornCardIds = SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsUnlockable(true).getIsPrismatic(false).getCardIds();
              ...
              for (i = 1; i <= 4; i++) {
                ...
                if (random < 0.74) {
                  // 74% chance for common
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,firstwatchCommonPrismaticChance,50));
                } else if (random < (0.74 + 0.16)) {
                  // 16% chance for rare
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,firstwatchRarePrismaticChance,50));
                } else if (random < (0.74 + 0.16 + 0.09)) {
                  // 9% chance for epic
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,firstwatchEpicPrismaticChance,50));
                } else {
                  // 1% chance for legendary
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,firstwatchLegendaryPrismaticChance,50));
                }
              }
              ...
              for (i = 1; i <= 4; i++) {
                ...
                if (random < 0.74) {
                  // 74% chance for common
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,wartechCommonPrismaticChance,50));
                } else if (random < (0.74 + 0.16)) {
                  // 16% chance for rare
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,wartechRarePrismaticChance,50));
                } else if (random < (0.74 + 0.16 + 0.09)) {
                  // 9% chance for epic
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,wartechEpicPrismaticChance,50));
                } else {
                  // 1% chance for legendary
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,wartechLegendaryPrismaticChance,50));
                }
              }
              ...
              for (i = 1; i <= 4; i++) {
                ...
                if (random < 0.74) {
                  // 74% chance for common
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,combinedCommonPrismaticChance,50));
                } else if (random < (0.74 + 0.16)) {
                  // 16% chance for rare
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,combinedRarePrismaticChance,50));
                } else if (random < (0.74 + 0.16 + 0.09)) {
                  // 9% chance for epic
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,combinedEpicPrismaticChance,50));
                } else {
                  // 1% chance for legendary
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,combinedLegendaryPrismaticChance,50));
                }
              }
              ...
              for (i = 1; i <= 4; i++) {
                ...
                if (random < 0.74) {
                  // 74% chance for common
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,fateCommonPrismaticChance,50));
                } else if (random < (0.74 + 0.16)) {
                  // 16% chance for rare
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Rare).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,fateRarePrismaticChance,50));
                } else if (random < (0.74 + 0.16 + 0.09)) {
                  // 9% chance for epic
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Epic).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,fateEpicPrismaticChance,50));
                } else {
                  // 1% chance for legendary
                  new_cards.push(randomCardFromCollectionWithoutDupes(SDK.GameSession.getCardCaches().getCardSet(this.cardSetId).getRarity(SDK.Rarity.Legendary).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(),new_cards,fateLegendaryPrismaticChance,50));
                }
              }
      ...
      .then(function(fbRootRef) {
        ...
        return FirebasePromises.remove(boosters.child(boosterPackId));}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
  ...
  static giveUserMissingCodexChapters(userId, systemTime){
    ...
    return txPromise = knex.transaction(function(tx) {
      ...
      .spread(function(progressionRow,codexInventoryRows) {
        ...
        const missingCodexChapterIds = _.filter(earnedCodexChapterIds, function(earnedCodexChapterId) {
        ...
        }}).then(results => this_obj.awardedChapterIds = _.filter(results, result => result !== null)).then(tx.commit)
      .catch(function(e){
  ...
  static _disenchantCards(trxPromise,trx,userId,cardIds,systemTime){
    ...
    const cardDataListReduced = _.reduce(cardIds,(function(memo,cardId){
      if (memo[cardId] == null) { memo[cardId] = {
        card_id:cardId,
        count:0
      }; }
      memo[cardId].count += 1;
    ...
    .then(function(userRow){
      ...
      return FirebasePromises.once(disenchantPromosRef,'value');}).then(function(promosSnapshot){

      this.disenchantPromos = promosSnapshot.val();
      return this.disenchantPromos;}).then(() => trx.select().from("user_cards").whereIn('card_id',cardIds).andWhere('user_id',userId).forUpdate())
    .then(function(cardCountRows){
      ...
      for (var disenchantedCardId in cardDataListReduced) {
        ...
        var countRow = _.find(cardCountRows,row => parseInt(row.card_id) === parseInt(disenchantedCardId));
      ...
      for (var cardId of Array.from(cardIds)) {
        ...
        try {
          ...
          var cardData = SDK.GameSession.getCardCaches().getIsCollectible(true).getCardById(parseInt(cardId));
          ...
          var baseCardId = cardData.getBaseCardId();
          var isPrismatic = SDK.Cards.getIsPrismaticCardId(cardData.getId());
      ...
      return Promise.resolve(this.rewards);}).then(function(rewards){

      this.final_wallet_spirit = this.userRow.wallet_spirit + this.total_spirit_gained;

      return InventoryModule.giveUserSpirit(trxPromise,trx,userId,this.total_spirit_gained,"disenchant");}).then(function(){
      ...
      return Promise.all(allQueries);}).then(function(){ return InventoryModule._refreshUserCardCollection(trxPromise,trx,userId,this.cardCountRows); })
    .then(function(){
  ...
  static disenchantDuplicateCards(userId,systemTime) {
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .bind(this_obj)
      .then(userRow => knex('user_card_collection').where('user_id',userId).first().transacting(tx).forUpdate()).then(function(collectionRow){
        this.cardIds = [];
        ...
        for (var cardId in collectionRow.cards) {
          ...
          if (cardData.count > 3) {
            if (cardsCache.getCardById(parseInt(cardId)) != null) {
              for (var i = 3, end = cardData.count, asc = 3 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        ...
        return Logger.module("InventoryModule").debug(`disenchantDuplicateCards() -> ${userId.blue} disenchanting ${util.inspect(this.cardIds)}.`);}).then(function(){ return InventoryModule._disenchantCards(txPromise,tx,userId,this.cardIds,NOW_UTC_MOMENT); })
      .then(function(data){ return _.extend(this,data); })
      .then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
  ...
  static craftCard(userId,cardId) {
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .then(function(userRow){
        ...
        return FirebasePromises.safeTransaction(fbRootRef.child("user-inventory").child(userId).child("wallet"),updateSpirit);}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
  ...
  static craftCosmetic(userId,cosmeticId) {
    ...
    .then(function(userRow){
      this.userRow = userRow;
      return tx("user_cosmetic_inventory").first().where("user_id",userId).andWhere("cosmetic_id",cosmeticId);}).then(function(cosmeticRow) {
  ...
  static giveUserCards(trxPromise, trx, userId, cardIds, sourceType, sourceId, memo) {
    ...
    const cardDataListReduced = _.reduce(cardDataList,(function(memo,cardData){

      if (memo[cardData.card_id] == null) { memo[cardData.card_id] = {
        user_id:userId,
    ...
    .spread(function(cardCountRows){
      ...
      for (cardId in cardDataListReduced) {
        ...
        var cardRow = _.find(cardCountRows,row => row.card_id === card.card_id);

        var updateCardPromise = null;
      ...
      return InventoryModule._refreshUserCardCollection(trxPromise,trx,userId,this.cardCountRows);}).then(function(cardCollectionRow){
  ...
  static addEmoteToUser(trxPromise, trx, userId, transactionType, emoteId, factionId){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.set(emote_entry,this.emotes_data);}).then(function(){
  ...
  static markCardAsReadInUserCollection(userId, cardId) {
    ...
    .then(() => Logger.module("InventoryModule").timeEnd(`markCardAsReadInUserCollection() -> user ${userId.blue} `.green+` marking card ${cardId} as READ`.green)).catch(function(error){
  ...
  static markAllCardsAsReadInUserCollection(userId) {
    ...
    .then(() => Logger.module("InventoryModule").timeEnd(`markAllCardsAsReadInUserCollection() -> user ${userId.blue} `.green+" marking all cards as READ".green)).catch(function(error){
  ...
  static markCardLoreAsReadInUserCollection(userId, cardId) {
    ...
    const txPromise = knex.transaction(function(tx){
      ...
      .bind({})
      .then(() => knex.select().from("user_card_lore_inventory").where({'user_id':userId,'card_id':cardId}).transacting(tx).forUpdate())
    ...
    .then(function(fbRootRef){
      ...
      return FirebasePromises.set(fbRootRef.child("user-inventory").child(userId).child("card-lore").child(cardId), cardLoreDataFB);}).then(() => Logger.module("InventoryModule").timeEnd(`markCardLoreAsReadInUserCollection() -> user ${userId.blue} `.green+` marking card ${cardId} lore as READ`.green)).catch(function(error){
  ...
  static softWipeUserCardInventory(userId,systemTime){
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .then(function(userRow){
        ...
        return tx("user_card_log").where('user_id',userId);}).then(function(cardLogRows){

        let cardCountRow, cardLog;
        const allPromises = [];
        ...
        return Promise.all(allPromises);}).then(function(){
        return InventoryModule._refreshUserCardCollection(txPromise,tx,userId,this.cardCountRows,true);}).then(() => tx("user_spirit_orbs_opened").update({
        "wiped_at":NOW_UTC_MOMENT.toDate()
      }).where('user_id',userId)).then(function(wipedCount){
        const allPromises = [];
        Logger.module("InventoryModule").debug(`softWipeUserCardInventory() -> adding ${wipedCount} orbs to ${userId.blue}`);
        for (let i = 1, end = wipedCount, asc = 1 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
      ...
      .then(tx.commit)
      .catch(tx.rollback);
      }).then(() => Logger.module("InventoryModule").timeEnd(`softWipeUserCardInventory() -> for user ${userId.blue}`)).catch(function(error){
  ...
  static claimFreeCardOfTheDay(userId,systemTime){
    ...
    const cardId = _.sample(SDK.GameSession.getCardCaches().getCardSet(randomCardSet).getRarity(SDK.Rarity.Common).getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds());
    ...
    .then(fbRootRef => FirebasePromises.set(fbRootRef.child("users").child(userId).child("free_card_of_the_day_claimed_at"),NOW_UTC_MOMENT.valueOf())).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
  ...
  static addArenaTicketToUser(trxPromise, trx, userId, transactionType, transactionId=null){
    ...
    .then(function(fbRootRef) {
      ...
      return FirebasePromises.set(tickets.child(ticketId),data);}).then(function(){
