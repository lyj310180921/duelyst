...
class RiftModule {
  ...
  static buyRiftTicketWithGold(userId) {
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .then(function(userRow){
        ...
        }}).then(() => RiftModule.addRiftTicketToUser(txPromise, tx,userId, "soft")).then(function(ticketId){
  ...
  static claimFirstFreeRiftTicket(userId){
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .spread(function(userRow,ticketRow, usedTicketRow,existingRun){
        ...
        return RiftModule.addRiftTicketToUser(txPromise,tx,userId,"free");}).then(function(ticketId) {
        this.ticketId = ticketId;
        return SyncModule._bumpUserTransactionCounter(tx,userId);}).then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
  ...
  static startRun(userId,ticketId) {
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .spread(function(userRow,ticketRow,existingRun,storedUpgradeRows){
        ...
        return Promise.all(storedUpgradeUpdatePromises);}).then(() => DuelystFirebase.connect().getRootRef())
      .then(function(fbRootRef) {
        ...
        return Promise.all(allfbPromises);}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function(){
  ...
  static chooseGeneral(userId, ticketId, generalId) {
    ...
    const txPromise = knex.transaction(function(tx){
      ...
      .then(function(fbRootRef) {
        ...
        return FirebasePromises.update(fbRootRef.child("user-rift-runs").child(userId).child(ticketId),fbRiftUpdateData);}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
  ...
  static updateRiftRunWithGameOutcome(userId,ticketId,isWinner,gameId,isDraw,damageDealt,gameSessionData) {
    ...
    .then(userRow => Promise.all([
      ...
      // Update redis
      return RiftManager.updateUserRunRiftRating(userId,ticketId,this.runData.rift_rating,NOW_UTC_MOMENT);}).then(() => DuelystFirebase.connect().getRootRef())
    .bind(this_obj)
    .then(function(fbRootRef) {
...
//      return Promise.resolve(riftRunRow)
    ...
    .bind(this_obj)
    .then(function(){
      return RiftModule._generateCardUpgradeChoices(txPromise,tx,this.userId,this.ticketId,this.factionId,this.cardIdToUpgrade,this.riftLevel);}).then(function(cardChoices){
