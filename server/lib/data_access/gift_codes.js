  ...
  static redeemGiftCode(userId,giftCode,systemTime){
    ...
    return txPromise = knex.transaction(function(tx){
      ...
      .spread(function(userRow, giftCodeRow, progressionRow){
        ...
        return exclusionCheckPromise;}).then(function(claimedGiftCodeWithMatchingExclusionRow) {
        let card_ids;
        ...
        if ((this.giftCodeRow.type === 'rewards') || (this.giftCodeRow.type === 'humble')) {
          ...
          if (spiritOrbs) {
            let asc, end;
            for (i = 0, end = spiritOrbs, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
          ...
          if (shimzarSpiritOrbs) {
            let asc1, end1;
            for (i = 0, end1 = shimzarSpiritOrbs, asc1 = 0 <= end1; asc1 ? i < end1 : i > end1; asc1 ? i++ : i--) {
          ...
          if (comboSpiritOrbs) {
            let asc2, end2;
            for (i = 0, end2 = comboSpiritOrbs, asc2 = 0 <= end2; asc2 ? i < end2 : i > end2; asc2 ? i++ : i--) {
          ...
          if (unearthedSpiritOrbs) {
            let asc3, end3;
            for (i = 0, end3 = unearthedSpiritOrbs, asc3 = 0 <= end3; asc3 ? i < end3 : i > end3; asc3 ? i++ : i--) {
          ...
          if (immortalSpiritOrbs) {
            let asc4, end4;
            for (i = 0, end4 = immortalSpiritOrbs, asc4 = 0 <= end4; asc4 ? i < end4 : i > end4; asc4 ? i++ : i--) {
          ...
          if (mythronSpiritOrbs) {
            let asc5, end5;
            for (i = 0, end5 = mythronSpiritOrbs, asc5 = 0 <= end5; asc5 ? i < end5 : i > end5; asc5 ? i++ : i--) {
          ...
          if (gauntletTickets) {
            let asc6, end6;
            for (i = 0, end6 = gauntletTickets, asc6 = 0 <= end6; asc6 ? i < end6 : i > end6; asc6 ? i++ : i--) {
