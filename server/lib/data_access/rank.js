...
class RankModule {
  ...
  static cycleUserSeasonRanking(userId,force,systemTime){
    ...
    .then(function(userRow) {
      ...
      return tx('user_rank_ratings').where('user_id',userId).andWhere('season_starting_at',this.userRow != null ? this.userRow.rank_starting_at : undefined).first().forUpdate();}).then(function(userRatingRow) {
    ...
    .then(function(fbRootRef){
      ...
      return Promise.resolve(allPromises);}).then(function(){
  ...
  static updateUserRankingWithGameOutcome(userId,isWinner,gameId,isDraw,systemTime) {
    ...
    .then(function(userRow) {
      ...
      allQueries.push(knex("user_games").where({'user_id':userId,'game_id':gameId}).update({
        ...
        rank_win_streak:rankData.win_streak
      }).transacting(tx).then(() => Logger.module("RankModule").debug(`updateUserRankingWithGameOutcome() -> user_games DONE. game_id:${gameId}`))
      );

      allQueries.push(knex("users").where('id',userId).update(updateParams).transacting(tx).then(() => Logger.module("RankModule").debug(`updateUserRankingWithGameOutcome() -> users DONE. game_id:${gameId}`))
      );

      Logger.module("RankModule").debug(`updateUserRankingWithGameOutcome() -> processing game and user objects. game_id:${gameId}`);

      return Promise.all(allQueries);}).then(() => DuelystFirebase.connect().getRootRef())
    .then(function(fbRootRef) {
      ...
      return SyncModule._bumpUserTransactionCounter(tx,userId);}).timeout(10000)
    .catch(Promise.TimeoutError, function(e){
    ...
    .then(fbRootRef => FirebasePromises.set(fbRootRef.child('user-games').child(userId).child(gameId).child('job_status').child('rank'),true)))).bind(this_obj)
    .then(function(){
      ...
      return Promise.resolve(this.rankData);}).finally(() => GamesModule.markClientGameJobStatusAsComplete(userId,gameId,'quests'));
  }
  ...
  static updateUsersRatingsWithGameOutcome(player1Id,player2Id,player1IsWinner,gameId,isDraw,player1IsRanked,player2IsRanked,systemTime) {
    ...
    .spread((player1UserRow, player2UserRow) => Promise.all([
      ...
      const player1RatingData = {
        ...
        volatility: (this.player1RatingRow != null ? this.player1RatingRow.volatility : undefined) || .06
      };
      ...
      const player2RatingData = {
        ...
        volatility: (this.player2RatingRow != null ? this.player2RatingRow.volatility : undefined) || .06
      };
      ...
      return Promise.all(redisPromises);}).then(function() {
    ...
    .then(function(fbRootRef) {
      ...
      return Promise.all(fbUpdatePromises);}).then(function() {
      clearTimeout(this.timeout);
  ...
  static claimRewardsForSeasonRank(userId,dateWithinSeason,systemTime){
    ...
    var txPromise = knex.transaction(function(tx){
      ...
      .then(function(fbRootRef){
        ...
        return rankHistoryFbPromise;}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
      .then(tx.commit)
      .catch(tx.rollback);
      }).bind(this_obj)
    .then(function() {
  ...
  static _getSeasonRankRewardMap(rank,seasonKey){
    ...
    let commonCards, epicCards, legendaryCards, randomCommonCardId, randomEpicCardId, randomIndex, randomLegendaryCardId, randomRareCardId, rareCards;
    const reward = {};
    reward.card_ids = [];
