...
class GamesModule {
  ...
  static newUserGame(userId,gameId,newGameParams){
    ...
    return knex.transaction(function(tx) {
      ...
      .then(function(fbRootRef) {
        ...
        return FirebasePromises.set(fbRootRef.child('user-games').child(userId).child(gameId),DataAccessHelpers.restifyData(newGameParams));}).then(() => Logger.module("GamesModule").log(`newUserGame() -> updated firebase u:${userId} g:${gameId} duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).timeout(15000) // timeout after 15 seconds
  ...
  static _addRewardIdToUserGame(tx,userId,gameId,rewardId,andUpdateAttributes){
    ...
    .then(function(gameRow){
      ...
      }}).then(() => DuelystFirebase.connect().getRootRef()).then(fbRootRef => FirebasePromises.set(fbRootRef.child('user-games').child(userId).child(gameId).child('rewards').child(rewardId),true));
  }
  ...
  static updateUserGame(userId,gameId,updateParams){
    ...
    return knex.transaction(function(tx) {
      ...
      .then(() => Logger.module("GamesModule").log(`updateUserGame() -> locked user (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).then(function(){
        ...
        return tx('user_games').where({'user_id':userId,'game_id':gameId}).first('game_id').forUpdate();}).then(() => Logger.module("GamesModule").log(`updateUserGame() -> locked game (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).then(function(){
        ...
        });}).then(() => Logger.module("GamesModule").log(`updateUserGame() -> updated game (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).then(() => DuelystFirebase.connect().getRootRef())
      .then(function(fbRootRef) {
        ...
        return FirebasePromises.update(fbRootRef.child('user-games').child(userId).child(gameId),DataAccessHelpers.restifyData(updateParams));}).then(() => Logger.module("GamesModule").log(`updateUserGame() -> updated firebase data (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).then(function(){
        Logger.module("GamesModule").log(`updateUserGame() -> bumping tx count (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`);
        return SyncModule._bumpUserTransactionCounter(tx,userId);}).then(() => Logger.module("GamesModule").log(`updateUserGame() -> bumped tx count (u:${userId} g:${gameId}) duration: ${moment.utc() - NOW_UTC_MOMENT}ms`)).timeout(10000)
      .catch(Promise.TimeoutError, function(e){
  ...
  static markClientGameJobStatusAsComplete(userId,gameId,jobName){
    ...
    .then(fbRootRef => FirebasePromises.set(fbRootRef.child('user-games').child(userId).child(gameId).child('job_status').child(jobName),true));
  }
