...
const createSinglePlayerGame = function(userId,name,gameType,deck,cardBackId,battleMapIndexesToSampleFrom,aiPlayerId,aiUsername,aiGeneralId,aiDeckId,aiDifficulty,aiNumRandomCards,ticketId,gameSetupOptions){
  ...
  .then(function(matchmakingStatus) {
    ...
    if (gameType !== GameType.BossBattle) {
      ...
      .then(function(fbRootRef) {
        ...
        for (var eventId in bossEventsData) {
          ...
          var matchingEventId = eventData.event_id;
          break;
        }
    ...
    return eventValidationPromise;}).then(function(){
    // get ai deck
    ...
    // generate game id
    return GameManager.generateGameId();}).then(function(gameId) { // save game to redis
    this.gameId = gameId;
    Logger.module("SINGLE-PLAYER").debug(`New Game ID: ${gameId}`);
    this.newGameSession.gameId = gameId;
    return GameManager.saveGameSession(gameId, this.newGameSession.serializeToJSON(this.newGameSession));}).then(function() { // assign the player to a server
    ...
    */
    ...
    if (['production', 'staging'].includes(config.get('env'))) {
      const server = config.get('matchmaking.defaultGameServer');
    ...
    return Promise.resolve(null);}).then(function(gameServer){
    ...
    // ...
    return GamesModule.newUserGame(userId,this.gameId,gameData);}).then(function(){ // send data back to the player
