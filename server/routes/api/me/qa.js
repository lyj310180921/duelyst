...
const QuestsModule = require('../../../lib/data_access/quests.coffee');
const GauntletModule = require('../../../lib/data_access/gauntlet.coffee');
const CosmeticChestsModule = require('../../../lib/data_access/cosmetic_chests.coffee');
const AchievementsModule = require('../../../lib/data_access/achievements.coffee');
const GiftCrateModule = require('../../../lib/data_access/gift_crate.coffee');
let SyncModule = require('../../../lib/data_access/sync.coffee');
const RankModule = require('../../../lib/data_access/rank.coffee');
SyncModule = require('../../../lib/data_access/sync.coffee');
const ShopModule = require('../../../lib/data_access/shop.coffee');
const TwitchModule = require('../../../lib/data_access/twitch.coffee');
...
const RankFactory = require('../../../../app/sdk/rank/rankFactory.coffee');
...
router.delete('/rank/history/:season_key/rewards', function(req,res,next){
  ...
  return knex("user_rank_history").where({'user_id':user_id,'starting_at':season_starting_at.toDate()}).update({
...
router.put('/rank/history/:season_key/top_rank', function(req,res,next){
  const user_id = req.user.d.id;
  const {
    season_key
  } = req.params;
...
router.put('/rank', function(req,res,next){
  ...
  .then(function(row){
    ...
    return knex("users").where({'id':user_id}).update(this.updateData);}).then(() => DuelystFirebase.connect().getRootRef()).then(function(fbRootRef) {
...
router.put('/rank_rating', function(req,res,next){
  let txPromise;
  const MOMENT_UTC_NOW = moment().utc();
  ...
  return txPromise = knex.transaction(function(tx){
    ...
    .then(function(userRankRatingRow) {
      // Update or insert
      ...
      }}).then(() => RankModule.updateAndGetUserLadderPosition(txPromise,tx,user_id,startOfSeasonMonth,MOMENT_UTC_NOW)).then(ladderPosition => newLadderPosition = ladderPosition).then(tx.commit)
    .catch(tx.rollback);
...
router.delete('/rank_rating', function(req,res,next){
  const MOMENT_UTC_NOW = moment().utc();
  const startOfSeasonMonth = moment(MOMENT_UTC_NOW).utc().startOf('month');
  const seasonStartingAt = startOfSeasonMonth.toDate();
...
router.get('/rank_rating', function(req,res,next){
  let txPromise;
  const MOMENT_UTC_NOW = moment().utc();
  const startOfSeasonMonth = moment(MOMENT_UTC_NOW).utc().startOf('month');
  const seasonStartingAt = startOfSeasonMonth.toDate();
...
router.get('/ladder_position', function(req,res,next){
  let txPromise;
  const MOMENT_UTC_NOW = moment().utc();
  const startOfSeasonMonth = moment(MOMENT_UTC_NOW).utc().startOf('month');
  const seasonStartingAt = startOfSeasonMonth.toDate();
...
router.post('/inventory/spirit', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
  const {
    amount
...
router.post('/inventory/gold', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
  const {
    amount
...
router.post('/inventory/premium', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
  const {
    amount
...
router.post('/inventory/rift_ticket', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
...
router.post('/inventory/cards', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
...
router.post('/inventory/card_set_with_spirit', function(req, res, next) {
  let txPromise;
  const user_id = req.user.d.id;
...
router.post('/inventory/fill_collection', function(req, res, next) {
  const user_id = req.user.d.id;
  ...
  .then(function(cardCollectionRow) {
    ...
    _.each(SDK.GameSession.getCardCaches().getIsCollectible(true).getIsUnlockable(false).getIsPrismatic(false).getCardIds(), function(cardId) {
      let numMissing;
      ...
      if (numMissing > 0) {
        return __range__(0, numMissing, false).map((i) =>
...
router.delete('/inventory/bloodborn', function(req, res, next) {
  ...
  .then(function(cardCollectionRow) {
    ...
    return Promise.map(this.ownedBloodbornCards,cardId => tx("user_cards").where("user_id",user_id).andWhere("card_id",cardId).delete());}).then(() => Promise.all([
...
router.delete('/inventory/unity', function(req, res, next) {
  ...
  .then(function(cardCollectionRow) {
    ...
    return Promise.map(this.ownedUnityCards,cardId => tx("user_cards").where("user_id",user_id).andWhere("card_id",cardId).delete());}).then(() => Promise.all([
...
router.delete('/quests/current', function(req, res, next) {
  ...
  .then(() => knex("users").where('id',user_id).update({
    ...
    return Promise.all([
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('quests')),
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("catch-up").child("current").child('quests')),
    ...
    ]);}).then(() => QuestsModule.generateDailyQuests(user_id)).then(() => res.status(200).json({}));
});
...
router.put('/quests/current', function(req, res, next) {
  ...
  .then(() => DuelystFirebase.connect().getRootRef()).then(function(fbRootRef) {
    ...
    return Promise.all([
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('quests')),
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("catch-up").child("current").child('quests'))
    ]);}).then(() => QuestsModule.generateDailyQuests(user_id)).then(() => Promise.all([
    ...
    return Promise.all([
      ...
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('quests').child(0).child("mulliganed_at")),
      FirebasePromises.remove(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('quests').child(1).child("mulliganed_at"))
    ]);}).then(() => res.status(200).json({}));
});

router.put('/quests/current/progress', function(req, res, next) {
  const user_id = req.user.d.id;
  const {
    quest_slots
  } = req.body;
...
router.put('/quests/generated_at', function(req, res, next) {
  ...
  .then(function(row){
    ...
    return Promise.all([
      FirebasePromises.set(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('updated_at'),this.newGeneratedAtMoment.valueOf()),
      FirebasePromises.set(this.fbRootRef.child("user-quests").child(user_id).child("daily").child("current").child('generated_at'),this.newGeneratedAtMoment.valueOf())
    ]);}).then(function() {
    return res.status(200).json({
...
router.post('/quests/setup_frostfire_2016', function(req, res, next) {
  const user_id = req.user.d.id;
...
router.post('/quests/setup_seasonal_quest', function(req, res, next) {
  const user_id = req.user.d.id;
...
router.post('/quests/setup_promotional_quest', function(req, res, next) {
  const user_id = req.user.d.id;
  const generateQuestsAt = req.body.generate_quests_at;
...
router.post('/matchmaking/time_series/:division/values', function(req, res, next) {
  const {
    division
  } = req.params;
  const {
...
router.post('/faction_progression/set_all_win_counts_to_99', function(req,res,next){
  const user_id = req.user.d.id;
  ...
  .then(function(rows){
    ...
    return Promise.all(all);}).then(() => res.status(200).json({}));
});

router.post('/faction_progression/add_level', function(req,res,next){
  const user_id = req.user.d.id;
  ...
  if (factionId != null) {
    ...
    .then(function(factionProgressionRow){
      ...
      }}).then(() => knex("user_faction_progression").where({'user_id': user_id, "faction_id": factionId}).first()).then(function(factionProgressionRow) {
      if ((factionProgressionRow == null)) {
        ...
        if (deltaXP <= 0) {
          ...
          const promises = [];
          for (let i = 0, end = winsNeeded, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
...
router.post('/faction_progression/set_all_levels_to_10', function(req,res,next){
  const user_id = req.user.d.id;
  ...
  .then(function(rows){
    ...
    return Promise.all(allPromises);}).then(() => knex("user_faction_progression").where('user_id',user_id)).then(function(factionRows) {
    const factionIds = _.map(FactionFactory.getAllPlayableFactions(), f => f.id);
    let winsPerFaction = [];
    for (var factionId of Array.from(factionIds)) {
      ...
      if ((row == null)) {
        ...
        var factionXp = row.xp;
        var xpForLevelTen = SDK.FactionProgression.levelXPTable[10];
        var neededXp = xpForLevelTen - factionXp;
        var xpPerWin = SDK.FactionProgression.winXP;
        if (neededXp > 0) {
          var neededWins = Math.ceil(neededXp / xpPerWin);
          winsPerFaction = winsPerFaction.concat(Array(neededWins).fill(factionId));
        }
      }
    }
    return Promise.each(winsPerFaction, factionId => UsersModule.updateUserFactionProgressionWithGameOutcome(user_id,factionId,true,generatePushId(),SDK.GameType.Ranked));}).then(() => res.status(200).json({})).catch(errorMessage => res.status(500).json({message: errorMessage}));
});

router.delete('/gauntlet/current', function(req,res,next){
  ...
  ])).then(() => res.status(200).json({})).catch(error => res.status(403).json({message:error.toString()}));
});
...
router.delete('/gauntlet/current/general', function(req,res,next){
  ...
  .then(function(gauntletData) {
    ...
    return res.status(200).json(this.gauntletData);}).catch(error => res.status(403).json({message:error.toString()}));
});

router.post('/gauntlet/progress', function(req,res,next){
  const userId = req.user.d.id;
...
router.post('/gauntlet/fill_deck', function(req,res,next){
  const userId = req.user.d.id;
  ...
  .then(function(gauntletData) {
    ...
    return recursiveSelect(gauntletData);}).catch(error => res.status(403).json({message:error.message}));
});

router.post('/gift_crate/winter2015', function(req,res,next){
  let txPromise;
  const userId = req.user.d.id;
...
router.post('/gift_crate/lag2016', function(req,res,next){
  let txPromise;
  const userId = req.user.d.id;
...
router.post('/cosmetic_chest/:chest_type', function(req,res,next){
  let txPromise;
  const userId = req.user.d.id;
...
router.post('/cosmetic_chest_key/:chest_type', function(req,res,next){
  let txPromise;
  const userId = req.user.d.id;
  const chestType = req.params.chest_type;
...
router.post('/cosmetic/:cosmetic_id', function(req,res,next){
  let txPromise;
  const userId = req.user.d.id;
  const {
    cosmetic_id
...
router.post('/referrals/events', function(req,res,next){
  const userId = req.user.d.id;

  const eventType = req.body.event_type;
  return knex("users").where('id',userId).first('referred_by_user_id')
  .then(userRow => ReferralsModule.processReferralEventForUser(userId,userRow.referred_by_user_id,eventType)).then(() => res.status(200).json({})).catch(err => next(err));
});

router.post('/referrals/mark', function(req,res,next){
  const userId = req.user.d.id;
  const {
    username
  } = req.body;
  return knex("users").where('username',username).first('id')
  .then(userRow => ReferralsModule.markUserAsReferredByFriend(userId,userRow.id)).then(() => res.status(200).json({})).catch(err => next(err));
});

router.put('/account/reset', function(req,res,next){
  const userId = req.user.d.id;
...
router.post('/daily_challenge', (req, res, next) => res.status(403).json({})
/*
...
router.post("/daily_challenge/completed_at", function(req, res, next) {
  const user_id = req.user.d.id;
  ...
  return Promise.all([
    ...
    return res.status(200).json(lastCompletedData);}).catch(function(error) {
...
router.post("/daily_challenge/passed_qa", function(req, res, next) {
  ...
  .then(function(fbRootRef) {
    ...
    return FirebasePromises.update(this.fbRootRef.child('daily-challenges').child(dateKey),{
      isQAReady: true
    });}).then(() => res.status(200).json({})).catch(function(error) {
...
// Updates the users current s rank rating
router.delete('/user_progression/last_crate_awarded_at', function(req,res,next){
...
router.post('/achievement/reset_and_complete', function(req, res, next) {
  const user_id = req.user.d.id;
  const {
    achievement_id
  } = req.body;
  ...
  .then(function(){
    ...
    return AchievementsModule._applyAchievementProgressMapToUser(user_id,achProgressMap);}).then(function(){
...
router.post('/migration/prismatic_backfill', function(req, res, next) {
  const user_id = req.user.d.id;
...
router.delete('/boss_event', function(req, res, next) {
  const bossEventId = "QA-Boss-Event";

  return DuelystFirebase.connect().getRootRef()
  .then(fbRootRef => FirebasePromises.remove(fbRootRef.child('boss-events').child(bossEventId))).then(function(){
    Logger.module("QA").log("Completed setting up qa boss event");
    return res.status(200).json({});
  });
});

router.delete('/boss_event/rewards', function(req, res, next) {
  const user_id = req.user.d.id;
  ...
  ]).then(() => SyncModule._syncUserFromSQLToFirebase(user_id)).then(function(){
...
router.put('/boss_event', function(req, res, next) {
  ...
  .then(function(fbRootRef) {
    this.fbRootRef = fbRootRef;
    return FirebasePromises.remove(this.fbRootRef.child('boss-events').child(bossEventId));}).then(function() {
    return FirebasePromises.set(this.fbRootRef.child('boss-events').child(bossEventId),bossEventData);}).then(function(){
    Logger.module("QA").log("Completed setting up qa boss event");
    return res.status(200).json({});
  });
});

router.put('/rift/duplicates', function(req, res, next) {
  const user_id = req.user.d.id;
...
router.get('/shop/charge_log', function(req, res, next) {
  const user_id = req.user.d.id;
