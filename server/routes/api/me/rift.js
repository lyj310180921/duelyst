...
router.get('/', function(req, res, next) {
  ...
  return knex("user_rift_runs").where('user_id',user_id).andWhere("win_count",">",0).orderBy("rift_rating",'desc').first()
...
router.get("/runs", function(req, res, next) {
  ...
  .then(rows => Promise.map(rows, riftRunRow => RiftModule.sanitizeRunCardChoicesIfNeeded(riftRunRow))).then(function(rows) {
...
router.post("/runs", function(req, res, next) {
  ...
  .then(function(data) {
    ...
    return res.status(200).json(DataAccessHelpers.restifyData(data));}).catch(function(error) {
    Logger.module("API").log(`ERROR starting rift run for user ${user_id.blue}`.red, util.inspect(error));
    return next(error);
  });
});

router.post("/runs/free", function(req, res, next) {
...
router.put("/runs/:ticket_id/general_id", function(req, res, next) {
  ...
  .then(function(data) {
    ...
    return res.status(200).json(data);}).catch(function(error) {
    Logger.module("API").log(`ERROR choosing rift general for user ${user_id.blue} ticket ${ticket_id}`.red, util.inspect(error));
    return next(error);
  });
});

router.post("/runs/:ticket_id/card_id_to_upgrade", function(req, res, next) {
...
router.post("/runs/:ticket_id/upgrade", function(req, res, next) {
  const result = t.validate(req.body.card_id, t.Number);
  if (!result.isValid()) {
    return res.status(400).json(result.errors);
  }
