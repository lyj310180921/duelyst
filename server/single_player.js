...
module.exports = io;
let port = config.get('sp_port');
server.listen(port, () => Logger.module("AI SERVER").log(`SP server started on port ${port}`));
...
var onGameTimeTick = function(gameId) {
  ...
  if (gameSession != null) {
    ...
    if (game.turnTimeRemaining <= 0) {
      ...
      return emitGameEvent(null,gameId,{type:EVENTS.turn_time, time: turnTimeRemainingInSeconds, timestamp: Date.now(), stepCount: totalStepCount});
    }
  }
};
...
var afterGameOver = function(gameId, gameSession, mouseAndUIEvents) {
  ...
  for (var player of Array.from(gameSession.players)) {
    ...
    if (playerId !== CONFIG.AI_PLAYER_ID) {
      ...
      var isDraw = (winnerId == null) ? true : false;
...
const shutdownHandler = function() {
  ...
  .then(function(reassign) {
    ...
    return Promise.map(ids, function(id) {
      ...
      return Consul.kv.get(`nodes/${node_name}/public_ip`);}).then(function(newServerIp) {
      ...
      return process.exit(0);}).catch(function(err) {
...
var ai_showUIForAction = function(gameId, action, actionDelayTime) {
  if (ai_isValidTurn(gameId) && ai_isValidActionForUI(gameId, action) && _.isNumber(actionDelayTime) && (actionDelayTime > 0.25)) {
    let card, cardIndex, handIndex, intent, isSignatureCard, moveToSelectDuration, moveToTargetDuration, pauseAtSelectDuration, pauseAtTargetDuration, selectPosition, targetPosition, uiAnimationDuration, uiDelayTime, unitToHover;
