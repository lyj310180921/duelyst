...
const getRequeueParams = function() {
  ...
  .then(function(v){
    ...
    return params;}).catch(error => // Just return the defaults if polling Consul fails
  defaults);
};
...
const logMatchMade = function(token1, token2) {
  let division1, division2, matchQuality;
  const now = Date.now();
...
const findOpponent = function(userId, lastOpponentId, rank, timeServed, deckValue, attempt, attemptInRanked, firstAttemptAt) {
  ...
  .then(function(params) {
    ...
    return Promise.all(queueSearchPromises);}).spread(function(casualPlayersWithinSearch, casualPlayersWithinDeckValueSearch, rankedPlayersWithinRankSearch, rankedPlayersWithinDeckValueSearch) {
        ...
        .then(function(searchData) {
          ...
          if (!opponent) {
            ...
            .then(function(opponentToken) { // TODO: We should validate results
              ...
              return Promise.all(removalPromises);}).then(function(results) { // TODO: We should spread and validate results
              // mark match made
              ...
              // We're done
              return done(null, {opponentName: this.token2.name});}).catch(Errors.NotFoundError, error => done(error)).catch(Errors.UnexpectedBadDataError, function(error){
